...
export default function SimulatorTab({ onShowToast }: SimulatorTabProps) {
  ...
  // Selected product category filtering
  const [activeCategory, setActiveCategory] = useState("전체");

  const categories = ["전체", "건강기능식품", "체중조절식품", "패키지 세트", "추천 테마"];
      ...
      <div className="bg-white border border-slate-200 p-6 sm:p-8 rounded-3xl shadow-sm space-y-4">
        <div className="flex items-center space-x-2">
          ...
          <h2 className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight">
            공유와 연결 주급 실시간 정산 시뮬레이터
          </h2>
        </div>
        <div className="bg-gradient-to-br from-teal-50 to-emerald-50 border border-teal-100 p-5 rounded-2xl text-teal-950 font-bold text-sm sm:text-base leading-relaxed flex items-start space-x-3">
          <Sparkles className="w-5 h-5 text-teal-600 shrink-0 mt-0.5 animate-pulse" />
          <div>
            <strong>핵심 정산 규칙 안내:</strong> 각 라인의 실질 누적 점수가{" "}
            <span className="text-teal-700 underline underline-offset-4 decoration-2">125점 이상일 때 부터</span>{" "}
            정산 수당이 매주 계산되며, 한쪽 라인이 미달하거나 125점 미만일 때는 수당 정산 대기 상태로 유지되어{" "}
            <span className="text-emerald-700 font-extrabold">남은 점수가 지워지지 않고 평생 안전하게 누적 이월</span>됩니다.
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-5 space-y-6">
          <div className="bg-white border border-slate-200/95 p-5 rounded-3xl shadow-sm space-y-5">
            ...
            {/* 제품 카드 목록 */}
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-3.5 max-h-[480px] overflow-y-auto pr-2 no-scrollbar border-t border-slate-100 pt-3">
                ...
                {filteredProducts.map((prod, index) => (
                  ...
                  >
                    <div>
                      ...
                      <span className="font-extrabold text-slate-900 text-sm block mt-0.5 leading-snug">
                        {prod.name}
                      </span>
                    ...
                    <div className="grid grid-cols-2 gap-1.5 pt-2 border-t border-dashed border-slate-200">
                      <button
                        ...
                        className="bg-white hover:bg-teal-50 active:scale-95 transition text-slate-800 text-[11px] py-1.5 rounded-lg border border-slate-200/80 font-bold hover:border-teal-300"
                      >
                        A(가족) +
                      </button>
                      <button
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          <div className="bg-gradient-to-r from-slate-950 via-teal-950 to-slate-900 text-white p-6 sm:p-8 rounded-3xl shadow-lg border border-slate-800 space-y-5">
            <div className="flex justify-between items-center border-b border-white/10 pb-3">
              <h3 className="font-black text-lg sm:text-xl text-teal-400">
                📊 매주 금요일 입금되는 내 주급 영수증
              </h3>
              <span className="text-slate-400 text-xs font-mono">STATUS: LIVE CALC</span>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div className="bg-white/5 p-4 rounded-2xl border border-white/5 flex flex-col justify-between">
                ...
                <span className={`text-[11px] block mt-1.5 ${bc2CP > 0 ? 'text-teal-300' : 'text-slate-400 font-medium'}`}>
                  {bc2Formula}
                </span>
                <span className="font-mono font-black text-lg text-emerald-400 mt-2.5 block">
                  {bc2CP.toFixed(1)} CP
                </span>
              </div>
              ...
              <div className="bg-white/5 p-4 rounded-2xl border border-white/5 flex flex-col justify-between">
                ...
                <span className={`text-[11px] block mt-1.5 ${bc3CP > 0 ? 'text-teal-300' : 'text-slate-400 font-medium'}`}>
                  {bc3Formula}
                </span>
                <span className="font-mono font-black text-lg text-emerald-400 mt-2.5 block">
                  {bc3CP.toFixed(1)} CP
              ...
              <div className="bg-white/5 p-4 rounded-2xl border border-white/5 flex flex-col justify-between">
                ...
                <span className={`text-[11px] block mt-1.5 ${bc1CP > 0 ? 'text-teal-300' : 'text-slate-400 font-medium'}`}>
                  {bc1Formula}
                </span>
                <span className="font-mono font-black text-lg text-emerald-400 mt-2.5 block">
                  {bc1CP.toFixed(1)} CP
            ...
            <div className="pt-4 border-t border-white/10 flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-4">
              ...
              <div className="sm:text-right bg-slate-900/50 p-4 rounded-2xl border border-white/15">
                <span className="text-teal-400 text-xs font-black block">예상 고정주급 (원화 환산)</span>
          ...
          {/* 3BC 계보 다이어그램 */}
          <div className="bg-white border border-slate-200 p-4 sm:p-6 rounded-3xl flex flex-col items-center relative w-full aspect-[1.12/1] sm:aspect-[1.25/1] max-w-[620px] mx-auto shadow-sm select-none">
            <h4 className="font-black text-slate-850 text-xs sm:text-base mb-2 relative z-10 bg-white px-3 shadow-none">
              🔗 내 비즈니스 센터 실시간 점수 누적 계보도
            </h4>
            ...
            <div className="absolute inset-0 w-full h-full pointer-events-none">
              <svg className="w-full h-full" style={{ zIndex: 0 }} viewBox="0 0 600 500" fill="none" preserveAspectRatio="none">
                ...
                {/* BC1 TO BC2 */}
                <line x1="300" y1="125" x2="180" y2="225" stroke="#0f766e" strokeWidth="2" strokeDasharray="5 4" />
                {/* BC1 TO BC3 */}
                <line x1="300" y1="125" x2="420" y2="225" stroke="#0f766e" strokeWidth="2" strokeDasharray="5 4" />
                {/* BC2 TO A */}
                <line x1="150" y1="275" x2="80" y2="375" stroke="#0284c7" strokeWidth="1.5" strokeDasharray="4 3" />
                {/* BC2 TO B */}
                <line x1="210" y1="275" x2="220" y2="375" stroke="#0284c7" strokeWidth="1.5" strokeDasharray="4 3" />
                {/* BC3 TO C */}
                <line x1="390" y1="275" x2="380" y2="375" stroke="#4f46e5" strokeWidth="1.5" strokeDasharray="4 3" />
                {/* BC3 TO D */}
                <line x1="450" y1="275" x2="520" y2="375" stroke="#5b21b6" strokeWidth="1.5" strokeDasharray="4 3" />
              </svg>
            </div>
            ...
            <div className="absolute inset-0 w-full h-full z-10">
              ...
              {/* BOTTOM D */}
              <div 
                className="absolute bg-purple-50/90 border border-purple-200 p-1 sm:p-2 rounded-xl shadow-sm text-center"
                style={{
                  left: "86.7%",
                  top: "80%",
                  transform: "translate(-50%, -50%)",
              ...
              >
                <span className="text-[6px] sm:text-[10px] text-purple-850 font-black block leading-none truncate">
                  D (친구)
                </span>
                <span className="font-mono font-black text-slate-900 text-[8px] sm:text-sm mt-0.5 sm:mt-1 block">
                  {lineD} P
